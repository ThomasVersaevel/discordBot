...
const { convertLolName, fetchApiEndpoint, getUserInfo } = require("../globals.js");
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let icon = `http://ddragon.leagueoflegends.com/cdn/${patchNr}/img/profileicon/${userData.profileIconId}.png`;
    ...
    for (var id = 0; id < matchIdData.length; id++) {
      //for in uses id as an iterator thus do data[id]
      ...
      outcome = matchData.info.participants[partIndex].win
        ? ":green_circle:"
        : ":red_circle:";

      let timestampMin = matchData.info.gameDuration / 0.6;
      ...
      let stamp =
        timestamp.substring(0, 2) + ":" + secondsString.substring(0, 2);
      let kda = partData.kills + "/" + partData.deaths + "/" + partData.assists;

      exampleEmbed.addField(
        outcome + " " + " " + kda,
        partData.championName + "  " + stamp,
        true
      );
    }
